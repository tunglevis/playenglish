...
<!--    center box-->
    <div class="eight columns">
        <div class="feed-course" style="overflow: hidden;border: 1px solid #ececec;">
            <form id="feed-form" method="post" enctype="multipart/form-data" action="<?php echo Yii::app()->createUrl("web/course/feedForm", array('id' => $course->id))?>">
                <ul style="border-bottom: 1px solid #ececec;margin: 10px;padding-bottom: 10px;font-size: 12px;">
                    <li style="display: inline-block;padding-right: 10px;">
                        <a class="active" role="button"><label for="feed-type-1">Tips English</label></a>
                    </li>&nbsp;<span style="font-size: 10px;">|</span>&nbsp;
                    <li style="display: inline-block;padding-left: 10px">
                        <a id="feed-image" role="button"><label for="feed-type-2">Add Photos</label></a>
                    </li>
                    <div style="display: none;">
                        <input type="radio" id="feed-type-1" name="feed-type" value="1" checked>
                        <input type="radio" id="feed-type-2" name="feed-type" value="2">
                    </div>
                </ul>
                <textarea title="What's on your mind?" name="feed-text" placeholder="What's on your mind?" role="textbox" aria-autocomplete="list" autocomplete="off" aria-expanded="false" id="u_0_w" tabindex="0" style="resize: none;height: 63px;min-height: 63px;border: none;"></textarea>
                <div class="upload-feed-image" style="border-top: 1px solid #ececec;padding: 10px 10px 0;display: none;">
                    <input type="file" id="feedImage" name="feed-image">
                    <img id="img_file" style="clear: both;display: block;margin: 0 auto;max-height: 100px;padding-top: 5px;">
                </div>
                <div id="feedSubmit">
                <button type="submit" style="float: right;margin:5px 10px 10px;">Gửi</button>
            </div>
            </form>
        </div>
...
<!--        content a feed course-->
        <?php if(isset($posts) && count($posts)>0):?>
            <div id="list-feeds">
                <?php foreach($posts as $key => $value):?>
                    ...
                    <div>
                        <a style="float: left;display: inline-block;"><img src="<?php echo User::model()->getAvatarUrl($value->id_send).'?'.uniqid()?>"></a>
                        <div style="display: inline-block;line-height: 20px;font-size: 12px;padding-left: 10px;">
                            <a style="display: block;color: #00aec8" href="<?php echo Yii::app()->createUrl('/web/user', array('user_name'=>str_replace(array(' ', '\\', '/', '-', '+', '*', '!', '@', '#','$','%','^', '&', '(',')', '=', '|'), '_', $value->name_send), 'id'=>$value->id_send))?>"><?php echo $value->name_send?></a>
                    ...
                    <div style="clear: both;display: block;text-align: justify;padding: 5px 0 10px;font-size: 13px;">
                        <?php if($value->image):?>
                            <img src="<?php echo $value->getUrlImage()?>" style="display: block;margin: 0 auto;">
                        <?php endif;?>
                        <p><?php echo $value->feed?></p>
                    </div>
                    <!--            comment-->
                        <div id="comment-feed-<?php echo $value->id?>" style="clear: both;display: block;padding-bottom: 15px">
                            <?php foreach($value->comment as $_key => $_val):?>
    ...
    <!--                form comment daily english-->
                    <form onsubmit="return add_comment($(this))" comment-id="<?php echo $value->id?>" method="post" enctype="multipart/form-data" action="<?php echo Yii::app()->createUrl("web/course/commentFeed", array('id'=>$course->id, 'comment_id' => $value->id))?>">
                        <div>
                            ...
                            <div style="display: none;">
                                <input type="file" id="feedImageComment-<?php echo $value->id?>" name="comment-image" onchange="getImgCover('feedImageComment-<?php echo $value->id?>')">
                            </div>
                            <label for="feedImageComment-<?php echo $value->id?>"><img style="float: right;padding-right: 5px;" src="<?php echo Yii::app()->baseUrl?>/themes/web/files/images/icons/1418291258_camera-32.png"></label>
                        </div>
...
<!--    right box-->
    <div class="five columns">
        <?php if(isset($course->notify) && count($course->notify) >0):?>
            <div class="right-box" style="box-shadow: 1px 2px 10px #aff">
                <h2 class="title">Thông báo<span class="line"></span></h2>
                <?php foreach($course->notify as $_notify_key => $_notify_val):?>
                    <span style="display: block;clear: both"><i class="icon-document" style="margin-right: 5px;height: 28px;width: 28px;display: inline-block;float: left;background-size: 100%;background-image: url(<?php echo Yii::app()->baseUrl?>/themes/web/files/images/icons/1418370168_sound-32.png)"></i><span style="font-size: 13px;"><?php echo $_notify_val->desc?></span></span>
                <?php endforeach;?>
            </div>
        <?php endif;?>
...
<?php if(isset($posts) && count($posts)>0):?>
    <?php $this->widget('ext.yiinfinite-scroll.YiinfiniteScroller', array(
        'contentSelector' => '#list-feeds',
        'itemSelector' => 'div.content-feed-course',
        'loadingText' => 'Loading...',
...
//        var f = files[0];
    ...
    function add_comment(element) {
        var comment_id = element.attr('comment-id');
        ...
        $.ajax({
            ...
            success:function(data){
                if(parseInt(data) != 1 && parseInt(data) != 0){
                    ...
                    that.find('#feedImageComment-'+comment_id).replaceWith(that.find('#feedImageComment-'+comment_id).val('').clone(true));
                    ...
                    var html = '<div class="comment-feed">'+
                        ...
                        '</div>'+
                        '</div>';
                    $('#comment-feed-'+comment_id).append(html).show('slow');
                }else if(data == 1){
                    that.find('.your-comment').val('');
                    that.find('#feedImageComment-'+comment_id).replaceWith(that.find('#feedImageComment-'+comment_id).val('').clone(true));
                    that.find(".img_file_comment").hide();
