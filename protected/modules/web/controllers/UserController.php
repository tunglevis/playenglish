        ...
        public function accessRules()
        {
            return array(
                array('allow',  // allow all users to perform 'index' and 'view' actions
                    'actions'=>array('page','login', 'forgot','captcha', 'create', 'verifyPhoneCreate', 'verifyPhoneForgot','pageinfo', 'pagefavorite', 'pagecoupon'),
                    'users'=>array('*'),
                ),
                array('allow', // allow authenticated user to perform 'create' and 'update' actions
                    'actions'=>array('logout', 'index', 'feed', 'update', 'manageEmail', 'managePhone', 'password', 'verifyPhoneAdd', 'ajaxUpdateEmailList', 'ajaxUpdatePhoneList','feedForm','commentFeed'),
                    'users'=>array('@'),
                ),
...
//            if($this->user) $this->redirect('/web/user/update');
            ...
            $criteria = new CDbCriteria();
            $criteria->addCondition('t.id_send = '.$id.' OR t.id_receive ='.$id);
            $criteria->compare('t.status', 'enable');
            $criteria->order = 't.id DESC';

            $total = MessageWall::model()->count($criteria);

            $pages = new CPagination($total);
            $pages->pageSize = 2;
            $pages->applyLimit($criteria);

            $posts = MessageWall::model()->with('comment')->findAll($criteria);
...
//        $this->nav_header = 'home';
            ...
            $feed = MessageWall::model()->with('comment')->findByPk($id);
            ...
            if(isset($_POST['LoginForm'])){
                ...
                if($model->validate() && $model->login())
                {
                    if(!Yii::app()->user->returnUrl || Yii::app()->user->returnUrl == '/'){
                        $this->redirect(Yii::app()->createUrl('/web/user', array('user_name'=>$model->username, 'id'=>Yii::app()->user->id)));
                    }else{
                        $this->redirect(Yii::app()->user->returnUrl);
                    }
                }
...
//            } 
            ...
            if(!$openid_id){
                $loginUrl = $facebook->getLoginUrl(array(
                    //                    'scope' => 'email,user_birthday,publish_stream,user_photos,user_relationships',
                    'scope' => 'email,user_birthday',
                    'redirect_uri' => 'http://meonho.net/tai-khoan/dang-nhap-voi-facebook',
                ));
...
//                $this->redirect($client->getRedirectUri());
         ...
         * @return 0 or 1
         */
        public function actionCommentFeed($id = null){
            if(!$id){
                echo 0;die;
            }
        ...
        public function actionFeedForm($id = null){
            ...
            if($check == 1){
                ...
                if(!$text){
                    ...
                    $model->content = htmlentities($text);
                    $model->id_send = $this->user->id;
                    $model->name_send = $this->user->name;
                    $model->is_read = 0;
                    $model->id_receive = $this->user->id == $id ? '' : $id;
                    $model->created = time();
